...
# Windows consoles default to cp1252, which cannot encode the emoji printed
# below. Without this the success message raises UnicodeEncodeError inside the
# try block, so a trunk that was created reports itself as a failure — and the
# obvious "retry" then creates a second one.
